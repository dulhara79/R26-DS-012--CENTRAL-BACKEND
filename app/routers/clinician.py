...
def create_subject(
    ...
    if db.get(models.Subject, subject_id) is None:
        ...
        db.flush()  # subject must exist before its alias / pairing code
        db.add(
            models.SubjectAlias(
                subject_id=subject_id, alias_type="mrn_hash", alias_value=mrn_hash
            )
        )
    ...
    db.commit()
    return schemas.CreateSubjectOut(
...
@router.get("/subjects")
def list_subjects(
    p: dict = Depends(security.require_clinician), db: Session = Depends(get_db)
):
    subjects = (
        db.execute(
            select(models.Subject)
            .where(models.Subject.clinician_id == p["sub"])
...
def get_support_set(
    ...
    return {
        "k": len(rows),
        "class_balance": {"anxiety": anxiety, "control": len(rows) - anxiety},
        "examples": [
            {
                "id": r.id,
                "label": r.label,
...
def add_support_example(
    subject_id: str,
    body: schemas.SupportExampleIn,
    p: dict = Depends(security.require_clinician),
    db: Session = Depends(get_db),
):
    _own(db, subject_id, p)
...
def timeline(
    ...
    notes = (
        db.execute(
            ...
            .where(models.ClinicalNote.subject_id == subject_id)
            .order_by(models.ClinicalNote.note_date.desc())
            .limit(20)
        )
        .scalars()
        .all()
    ...
    return {
        ...
        "fusion": (
            ...
            if fusion is None
                ...
                "provisional": fusion.provisional,
...
def subject_audit(
    ...
    rows = (
        db.execute(
            ...
            .where(models.AuditLog.subject_id == subject_id)
