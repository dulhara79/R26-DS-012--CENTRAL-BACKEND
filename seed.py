...
SUPPORT = [
    ...
    (
        "Routine review. Mood euthymic, sleep adequate, no worry reported.",
        "control",
        90,
    ),
...
def behaviour_nodes(rng: random.Random) -> dict:
    ...
    return {"morning": slot(), "afternoon": slot(), "evening": slot(), "night": slot()}


def physio_features(rng: random.Random) -> dict:
    return {
        "hr_mean": round(rng.uniform(62, 96), 1),
...
async def main() -> None:
    ...
    for label, plan in scenarios.items():
        ...
        db.flush()  # subject must exist before anything references it
        db.add(
            models.SubjectAlias(
                subject_id=sid, alias_type="mrn_hash", alias_value=mrn_hash
            )
        )
        ...
        if plan["behaviour_days"]:
            for i in range(plan["behaviour_days"]):
                day = (
                    (now - timedelta(days=plan["behaviour_days"] - i))
                    .date()
                    .isoformat()
                )
                db.add(
                    models.BehaviorDay(
                        subject_id=sid, day=day, nodes=behaviour_nodes(rng)
                    )
                )
            db.commit()
        ...
        verdict = (
            f"composite {fusion.composite_score} ({fusion.band})"
